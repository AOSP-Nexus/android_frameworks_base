...
public class ImmersiveTile extends QSTile<QSTile.BooleanState> {
    ...
    protected void handleUpdateState(BooleanState state, Object arg) {
        ...
        state.visible = true;
        state.label = mContext.getString(R.string.quick_settings_immersive_label);
        if (immersiveMode) {
            state.icon = ResourceIcon.get(R.drawable.ic_qs_immersive_full);
            state.contentDescription =  mContext.getString(
                    R.string.accessibility_quick_settings_immersive_full);
        } else {
            state.icon = ResourceIcon.get(R.drawable.ic_qs_immersive_off);
            state.contentDescription =  mContext.getString(
                    R.string.accessibility_quick_settings_immersive_off);
        }
    }
    ...
    protected String composeChangeAnnouncement() {
        if (mState.value) {
            return mContext.getString(R.string.accessibility_quick_settings_immersive_full);
        } else {
            return mContext.getString(R.string.accessibility_quick_settings_immersive_off);
        }
    }
